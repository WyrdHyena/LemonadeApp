...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        when (lemonadeState) {
            ...
            /*transition to SQUEEZE, set the lemonSize (the number of squeezes needed)
             by calling the pick() method, and setting the squeezeCount (the number of times
             the user has squeezed the lemon) to 0.*/
            "squeeze" -> {
                ++squeezeCount
                --lemonSize
                if (lemonSize == 0) {
            ...
            /*Increment the squeezeCount by 1 and decrement the lemonSize by 1.
             Remember that a lemon will require a variable number of squeezes before the app
             can transition its state. Transition to the DRINK state only if the new lemonSize
             is equal to 0. Otherwise, the app should remain in the SQUEEZE state.*/
            "drink" -> {
                lemonSize = -1
                lemonadeState = "restart"
            }
            /*Transition to the RESTART state and set the lemonSize to -1.*/
            "restart" -> {
                lemonadeState = "select"
            }
            /*Transition back to the SELECT state.*/
        }
        setViewElements()
